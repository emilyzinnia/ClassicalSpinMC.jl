...
@testset "Energy test" begin
    @testset "Zeeman" begin
        ...
        lat = Lattice((1,1), U, S=1.0)
        lat.spins .= [1.0, 0.0, 0.0]

        @test -1.0 == total_energy(lat)
        @test -h == get_local_field(lat, 1)
    end

    @testset "Onsite" begin
        U = Square()
        addOnSite!(U, 1, collect(1.0I(3)))
        lat = Lattice((1,1), U, S=1.0)
        lat.spins .= [1.0, 1.0, 1.0]
        @test 3.0 == total_energy(lat)
        @test [2.0, 0.0, 0.0] == get_local_field(lat, 1)
    end
